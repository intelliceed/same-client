...
function * submitSaga ({ payload }:{payload: SubmitPayload}) {
  ...
  try {
    yield call(PUB.post, '/auth/reset-password', { password: payload.password, token: qs.parse(history.location.search, { ignoreQueryPrefix: true })?.token });
    yield call(toast.success, 'Your password were successfully changed, you can login using new password');
